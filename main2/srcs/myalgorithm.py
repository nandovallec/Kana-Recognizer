...
class MyAlgorithm():
    ...
    def predict(self,img):
        ...
        # print("mmm")
        # d1 = np.linalg.norm(self.model - feat, axis=1)  # measure distance
        dist = (np.sqrt(np.cumsum(np.square(self.model-feat), axis=1)))[:,-1]

        # if(np.array_equal(d1,dist)):
        #     print("YES")


        # d2 = np.linalg.norm(self.model - feat2, axis=1)  # measure distance
        dist2 = (np.sqrt(np.cumsum(np.square(self.model-feat2), axis=1)))[:,-1]

        # if (np.array_equal(d2, dist2)):
        #     print("YES2")

        # d3 = np.linalg.norm(self.model - feat3, axis=1)  # measure distance
        dist3 = (np.sqrt(np.cumsum(np.square(self.model-feat3), axis=1)))[:,-1]

        # if (np.array_equal(d3, dist3)):
        #     print("YES3")
